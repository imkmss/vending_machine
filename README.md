...
| 5단계 | 웹 인터페이스 (Flask + Chart.js 대시보드, REST API 5종) | ✅ 완료 |
...
├── server/
│   ├── __init__.py
│   ├── server_main.py             ← VendingServer (accept 루프, 웹 서버 시작)
│   ├── server_handler.py          ← ClientHandler, ClientRegistry (per-connection 스레드)
│   ├── server_db.py               ← SalesDataStore (BST, thread-safe)
│   ├── sync_manager.py            ← SyncManager (Server ↔ Server TCP 동기화)
│   ├── web_server.py              ← Flask 웹 대시보드 (기본 포트 8080)
│   └── templates/
│       └── dashboard.html         ← Chart.js 대시보드 HTML
└── client/
    ├── core/
    ...
    │   ├── drink_config.json      ← 음료 이름·가격 설정 (자동 생성)
    ...
    └── ui/
        ...
        └── admin_window.py        ← 관리자 화면 (PyQt5, 사이드바 7탭)
```
...
| 가격 변경 | drink_id + 새 가격 입력 → `{이름}: {기존가}원 → {새가}원` 결과 표시 |
...
`python3 main.py VM_01 --all` 로 실행 시 자판기 화면과 관리자 화면이 함께 열리며, 두 창은 다음과 같이 연동됩니다.

| 동작 | 설명 |
|------|------|
| 자판기 운영 중단 | 관리자 로그인 시 `sales.setEnabled(False)`, 관리자 창 닫으면 `True` 복원 |
| ChangeReserve 공유 | `AdminWindow(reserve=sales.reserve)` — 두 창이 동일한 잔돈 보유고 인스턴스를 참조, 화폐 현황 탭이 실시간 잔돈 상태를 반영 |
| 이름·가격 변경 즉시 반영 | `admin.inventory_changed` 시그널 → `sales._refresh_drinks()` 연결, 관리자 화면에서 변경하면 자판기 화면 버튼이 즉시 갱신 |
...
- 동기화 방식: **TCP 소켓** (전용 sync 포트)을 사용하여 SYNC(0x07) / SYNC_ACK(0x08) 메시지로 처리합니다. 재연결·재전송 로직 내장.

```
Server1  ◀──── TCP Socket (SYNC/SYNC_ACK, 전용 sync 포트) ────▶  Server2
         (실시간 양방향 동기화)
```
...
| 파일 | 클래스 | 역할 |
|------|--------|------|
| `server_main.py` | `VendingServer` | 소켓 bind/listen, accept 루프, SyncManager·웹 서버 초기화 |
| `server_handler.py` | `ClientHandler`, `ClientRegistry` | per-connection 스레드, 메시지 파싱·디스패치, 이름 변경 명령 전송 |
| `server_db.py` | `SalesDataStore` | BST 기반 매출 저장, thread-safe (Lock) |
| `sync_manager.py` | `SyncManager` | Server ↔ Server 실시간 TCP 동기화 (전용 sync 포트) |
| `web_server.py` | Flask app | 웹 대시보드 (기본 8080), REST API 5종 (`/api/clients`, `/api/monthly`, `/api/daily`, `/api/breakdown`, `/api/inventory`) |
...
| ③ | 각 자판기의 실시간 재고현황 파악 |
| ④ | 음료 재고 부족 시 관리자에게 알림 메시지 전송 |
| ⑤ | 각 자판기의 음료 이름 변경 명령 전달 (ClientRegistry) |
| ⑥ | Server1 ↔ Server2 실시간 데이터 동기화 |
| ⑦ | 웹 대시보드 (Flask, 포트 8080) — 매출·재고·음료별 분석 차트 조회 |
...
**Server ↔ Server: TCP 소켓 (전용 sync 포트)**
- 전용 sync 포트를 통해 SYNC(0x07) / SYNC_ACK(0x08) 메시지로 실시간 동기화. 전송 실패 시 큐에 재적재 후 재연결 시 재전송.
...
#### 3) TCP Sync 메시지 구조 (Server ↔ Server)

클라이언트 ↔ 서버와 **동일한 12 byte 헤더** 포맷을 공유합니다. TYPE 필드로 SYNC/SYNC_ACK를 구분합니다.
...
| CLIENT_ID | 2 bytes | 원본 자판기 식별자 |
| CHECKSUM | 2 bytes | 페이로드 16비트 합산 체크섬 |
| PAYLOAD | 가변 | `{"client_id", "date", "price"}` JSON |

SYNC_ACK는 페이로드 없음 (PAYLOAD_LEN = 0). ACK 미수신 시 항목을 큐 앞에 재적재하고 5초 후 재연결.
...
│  ┌──────────────────┐  TCP Socket     ┌────────────────┐  │
│  │    Server 1      │◀─────────────▶│   Server 2     │  │
│  │  수신·파싱·DB저장  │ (SYNC/SYNC_ACK) │ 수신·파싱·DB저장 │  │
